...
for(i in bin.files){
	...
	samp500 <- sample(unique(df$key_id), 500)
	...
	{ 
	### Plot the foods
	pdf(file = file.path("plots",paste(food.i,".pdf")))
	print(ggplot(df.sum) +
	...
	print(ggplot(filter(df, key_id %in% samp500)) +
		geom_path(aes(x=x, y=y, group = stroke), size = 0.001) +
